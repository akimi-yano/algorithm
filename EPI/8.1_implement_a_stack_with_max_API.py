...
class Stack:
    ...
    def push_to_bottom(self,val):
        if len(self.storage)==0 or self.storage[len(self.storage)-1]<val:
...
new_stack = Stack()
new_stack.push_to_bottom(1).push_to_bottom(2).push_to_bottom(3).push_to_bottom(4).push_to_bottom(5)
