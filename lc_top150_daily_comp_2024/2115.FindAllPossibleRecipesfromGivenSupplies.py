...
class Solution:
    def findAllRecipes(self, recipes: List[str], ingredients: List[List[str]], supplies: List[str]) -> List[str]:
        '''
        1) For each recipe, count its non-available ingredients as in degree; 
        ...
        3) Use topogical sort to decrease the in degree of recipes, 
        ...
        for rcp, ings in zip(recipes, ingredients):
            not_available = 0
        ...
        for rcp in ans:
            # Removes (pop) rcp from ingredient_to_recipe, which is a dictionary that maps an ingredient to the recipes that require it.
