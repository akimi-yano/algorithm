...
class Solution:
    def minOperationsMaxProfit(self, customers: List[int], boardingCost: int, runningCost: int) -> int:
        ...
        waiting = 0
        best_time = -1 
        for num_customer in customers:
            
            waiting += num_customer
            # print(total_num_customer)
            if waiting >= 4:
                total_num_rotate += 1 
                total_num_customer += 4
                waiting -= 4
                if total_num_customer * boardingCost - total_num_rotate * runningCost > best:
                    best = total_num_customer * boardingCost - total_num_rotate * runningCost
                    best_time = total_num_rotate
                
            else:
                total_num_rotate += 1 
                total_num_customer += waiting
                waiting -= waiting
                if total_num_customer * boardingCost - total_num_rotate * runningCost > best:
                    best = total_num_customer * boardingCost - total_num_rotate * runningCost
                    best_time = total_num_rotate
