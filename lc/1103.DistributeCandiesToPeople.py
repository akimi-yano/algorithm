...
# This solution works:
# I initially misunderstood that we needed to end when it hits  2*num_people but no ! this process  will continue 
...
# cleaned up version: there might be better ways than this ! 
class Solution:
    def distributeCandies(self, candies: int, num_people: int) -> List[int]:
        ans = [0 for _ in range(num_people)]
        k = 0
