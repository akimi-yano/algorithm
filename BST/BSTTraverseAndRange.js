...
class BST {
    ...
    range(node = this.root){
        return this.recursiveGetMax(node) - this.recursiveGetMin(node);
    }

    // find the range of a given subtree. use root if no subtree is passed.
    // the range is the max minus the min.

    // preorder traversal
    // log nodes in top to bottom order, so that you may copy the tree
    ...
    // in order 'smallest to largest' sorted output smallest to greatest
...
console.log(tree.inOrderArray(tree.root))

console.log("PREORDERARR")
console.log(tree.preOrderArray(tree.root))

console.log("POSTORDERARR")
console.log(tree.postOrderArray(tree.root))
